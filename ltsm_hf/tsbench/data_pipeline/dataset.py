...
class TSTokenDataset(Dataset):
    def __init__(
        ...
        config = ChronosConfig(
            ...
            tokenizer_kwargs=dict(low_limit=-2.0, high_limit=2.0),
            n_tokens=n_tokens,
    ...
    def __getitem__(self, index):
        ...
        seq_x = self.data[sequence_index][x_begin:x_end]
        seq_x = torch.from_numpy(np.expand_dims(seq_x, -1))
        x_token_ids, attention_mask, scale_x = self.tokenizer.input_transform(seq_x)
        seq_y = torch.from_numpy(np.expand_dims(self.data[sequence_index][y_begin:y_end], -1))
        x_token_ids, attention_mask, scale = self.tokenizer.input_transform(seq_x)
...
class TSPromptDatasetTXT(Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq_x = np.concatenate((self.data[sequence_index][x_begin:x_end], prompt_index))
        # seq_x = self.data[sequence_index][x_begin:x_end]
        seq_x = torch.from_numpy(np.expand_dims(seq_x, -1))
        seq_y = torch.from_numpy(np.expand_dims(self.data[sequence_index][y_begin:y_end], -1))
