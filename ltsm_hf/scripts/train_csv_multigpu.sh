...
for lr in 1e-3 
    do
        ...
        CUDA_VISIBLE_DEVICES=3 python3 main_hf.py --model_id test_run --model_name_or_path meta-llama/Llama-2-7b-hf --train_epochs ${epoch} --batch_size 100 --pred_len ${pred_len} --gradient_accumulation_steps 64 --data_path ${TRAIN} --test_data_path ${INIT_TEST} --test_data_path_list ${TEST} --prompt_data_path ${PROMPT} --freeze ${freeze} --learning_rate ${lr} --downsample_rate ${downsample_rate} --output_dir ${OUTPUT_PATH}
    done
done
