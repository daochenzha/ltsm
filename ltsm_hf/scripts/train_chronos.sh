...
for lr in 1e-3
    do
        ...
        downsample_rate=20
        freeze=0
        OUTPUT_PATH="output/Chronos_csv_lr${lr}_loraFalse_down${downsample_rate}_freeze${freeze}_e${epoch}_pred${pred_len}/"
        CUDA_VISIBLE_DEVICES=1,2 python3 main_hf.py --model_id  test_run --model Chronos --train_epochs ${epoch} --batch_size 20 --pred_len ${pred_len} --gradient_accumulation_steps 64 --data_path ${TRAIN} --test_data_path ${INIT_TEST} --test_data_path_list ${TEST} --prompt_data_path ${PROMPT} --freeze ${freeze} --learning_rate ${lr} --downsample_rate ${downsample_rate} --output_dir ${OUTPUT_PATH}
    done
done
